...
if __name__ == '__main__':
    ...
    for patient_name in patient_names:
        ...
        # resample md
        md_file = os.path.join(nii_root, patient_name, 'md.nii.gz')
        sitk_img_md = sitk.ReadImage(md_file)
        md_r_file = os.path.join(nii_root, patient_name, 'md_r.nii.gz')
        if not os.path.exists(md_r_file):
            sitk_img_md_r = resample_sitk_image_by_reference(sitk_img_roi1, sitk_img_md)
            sitk.WriteImage(sitk_img_md_r, md_r_file)
        ...
        data_dict['ID'].append(patient_name)
        data_dict['Image'].append(md_r_file)
        data_dict['Mask'].append(roi_1_file)
        data_dict['Type'].append('md_roi_1')

        data_dict['ID'].append(patient_name)
        data_dict['Image'].append(md_r_file)
        data_dict['Mask'].append(roi_ref_file)
        data_dict['Type'].append('md_roi_ref')
